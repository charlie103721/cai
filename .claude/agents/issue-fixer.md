...
3. **Reproduce first.** Write a test that reproduces the bug and **fails** against current `main` — this both proves the bug and becomes the regression test. Study neighboring `server/features/*`, the "Locked decisions" at the top of its roadmap file (`features-1.md`), and `AGENT.md` for the established patterns.
