...
import { MAX_EMAIL_LENGTH, MAX_PASSWORD_LENGTH } from "@/lib/auth-constraints";
...
export default function LoginPage() {
  ...
  // Without the isFetching guard, HMR or StrictMode re-mounts can serve
  // stale isAuthenticated=true from React Query cache, causing a spurious
  // redirect chain: /login -> / -> /signup.
  useEffect(() => {
    ...
    <main className="min-h-screen flex items-center justify-center bg-background p-4">
      <Card className="w-full max-w-[400px]">
        <CardHeader className="text-center">
          <h1 className="leading-none font-semibold text-2xl">Welcome back</h1>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <p className="text-center text-sm text-muted-foreground">
              ...
              <Link
                to={`/signup${returnTo !== DEFAULT_AUTH_REDIRECT ? `?returnTo=${encodeURIComponent(returnTo)}` : ""}`}
